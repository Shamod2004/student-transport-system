...
const Journey = () => {
  ...
  const redirectToBooking = (bus, event) => {
    if (event) {
    ...
    const fallbackDate = bus?.departureDate
      ? new Date(bus.departureDate).toISOString().split('T')[0]
      : '';
    const selectedDate = date || fallbackDate;

    bookingUrl.searchParams.set('routeId', bus?._id || bus?.busId || '');
    bookingUrl.searchParams.set('routeName', bus?.routeName || `${bus?.departureLocation || ''} to ${bus?.arrivalLocation || ''}`);
    bookingUrl.searchParams.set('busId', bus?.busId || '');
    bookingUrl.searchParams.set('from', bus?.departureLocation || '');
    bookingUrl.searchParams.set('to', bus?.arrivalLocation || '');
    bookingUrl.searchParams.set('date', selectedDate);
    bookingUrl.searchParams.set('departureDate', selectedDate);
    bookingUrl.searchParams.set('travelDate', selectedDate);
    bookingUrl.searchParams.set('departureTime', bus?.departureTime || '');
    bookingUrl.searchParams.set('arrivalTime', bus?.arrivalTime || '');
    bookingUrl.searchParams.set('duration', bus?.duration || '');
    const normalizedPrice = Number(
      bus?.price ?? bus?.fare ?? bus?.ticketPrice ?? bus?.amount ?? ''
    );
    bookingUrl.searchParams.set(
      'price',
      Number.isFinite(normalizedPrice) ? String(normalizedPrice) : ''
    );
    bookingUrl.searchParams.set('busType', bus?.busType || '');
    bookingUrl.searchParams.set('routeNumber', bus?.routeNumber || bus?.routeNo || bus?.routeName || bus?.busId || '');
    bookingUrl.searchParams.set('busName', bus?.busName || bus?.busModel || bus?.busId || bus?.routeName || '');
              ...
              <>
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <button
                      onClick={(event) => redirectToBooking(bus, event)}
      ...
      <AnimatePresence>
        {isRedirecting && (
          <motion.div
            ...
            className="fixed inset-0 z-[1200] bg-gradient-to-br from-slate-950/75 via-blue-950/60 to-slate-900/75 backdrop-blur-md"
          >
            <motion.div
              initial={{ opacity: 0, scale: 0.9 }}
              animate={{ opacity: [0.25, 0.55, 0.25], scale: [0.9, 1.1, 0.9] }}
              transition={{ duration: 2.2, repeat: Infinity, ease: 'easeInOut' }}
              className="pointer-events-none absolute left-1/2 top-1/2 h-72 w-72 -translate-x-1/2 -translate-y-1/2 rounded-full border border-cyan-300/25"
            />
            <div className="flex h-full w-full items-center justify-center">
              <motion.div
                ...
                className="w-[92%] max-w-md rounded-2xl border border-white/20 bg-black/40 p-5 text-white shadow-2xl"
              >
                <div className="mb-3 flex items-center justify-between">
                  <p className="text-xs uppercase tracking-[0.18em] text-blue-100/85">WayGo Transfer</p>
                ...
                <div className="mb-4 flex items-center gap-3">
                  <motion.div
                    ...
                    className="flex h-9 w-9 items-center justify-center rounded-lg bg-blue-500/20"
                  >
                    <FaBus className="text-blue-100" />
                  </motion.div>
                  <div>
                    <p className="text-sm font-semibold">Redirecting to booking</p>
                    <p className="text-xs text-slate-200/85">Building your route experience</p>
                  </div>
                </div>

                <div className="mb-3 overflow-hidden rounded-md border border-white/10 bg-white/5 px-2 py-2">
                ...
                <div className="mt-3 flex items-center gap-1 text-[11px] text-slate-200/80">
                  <span>Finalizing your trip portal</span>
                  <motion.span animate={{ opacity: [0.25, 1, 0.25] }} transition={{ repeat: Infinity, duration: 1.1 }}>.</motion.span>
                  <motion.span animate={{ opacity: [0.25, 1, 0.25] }} transition={{ repeat: Infinity, duration: 1.1, delay: 0.16 }}>.</motion.span>
                  <motion.span animate={{ opacity: [0.25, 1, 0.25] }} transition={{ repeat: Infinity, duration: 1.1, delay: 0.32 }}>.</motion.span>
                </div>
              </motion.div>
            </div>
          </motion.div>
